...
{
    public class CreatePaymentCommand : IRequest<ApiResponseViewModel>
    {
        ...
        public class CreatePaymentCommandHandler : IRequestHandler<CreatePaymentCommand, ApiResponseViewModel>
        {
            ...
            public CreatePaymentCommandHandler(RewardsDBContext rewardsDBContext, VodusV2Context vodusV2Context, IOptions<AppSettings> appSettings)
            {
            ...
            public async Task<ApiResponseViewModel> Handle(CreatePaymentCommand request, CancellationToken cancellationToken)
            {
                ...
                try
                {
                    var firstName = "";
                    var lastName = "";
                    if (request.Remark != "TEST PAYMENT")
                    {
                        if (!IsResponseSignatureValid(request.Signature, request.MerchantCode, request.PaymentId, request.RefNo.ToString(), request.Amount.ToString(), request.Currency, request.Status))
                        {
                            apiResponseViewModel.Message = "Invalid request. Incorrect signature";
                    ...
                    //  Update Order Status
                    var order = _rewardsDBContext.Orders.Include(x => x.OrderItems).Include(x=> x.OrderShopExternal).ThenInclude(x=> x.OrderItemExternal).Where(x => x.Id == request.RefNo).FirstOrDefault();
                    ...
                    string[] orderExternalReceiptViewList = new string[order.OrderShopExternal.SelectMany(x=> x.OrderItemExternal).ToList().Count];
                                            ...
                                            "<table style='width:100%;'> " +
                                               "<tbody style='padding:0 15px;font-size:16px;'>  " +
                                                   "<tr>" +
                                                      ...
                                                      $"<td width='50%' style='text-align: right;'>{ item.Points * Math.Ceiling((decimal)quantity)} </td> " +
                                                   "</tr>  " +
                                                   "<tr>  " +
                    ...
                    foreach (var shop in order.OrderShopExternal)
                    {
                        ...
                        foreach (var item in shop.OrderItemExternal)
                        {
                            ...
                            orderExternalReceiptViewList[j] = $"<table border='0' cellpadding='0' cellspacing='0' width='100%' class='x_478386741tableButton'>" +
                                         "<tbody style = 'font-family:Calibri'>" +
                                         "<tr>" +
                                         "<td align='center' valign='top' style='padding-top:20px; padding-bottom:20px; width:100%;'>" +
                                         "<table style = 'width:100%; max-width :  800px; '>" +
                            ...
                            orderReceiptView += orderExternalReceiptViewList[j];
                            j++;
                        }
                    }
                    ...
                    foreach (var item in orderExternalReceiptViewList)
                    {
                        //  Generate notification email to merchant 
                        var sendGridClientForMerchant = new SendGridClient(appSettings.Value.Mailer.Sendgrid.APIKey);
                        var msgForMerchant = new SendGridMessage();
                        ...
                        msgForMerchant.SetFrom(new EmailAddress(appSettings.Value.Emails.Noreply, "Vodus No-Reply"));
                        msgForMerchant.SetSubject("Vodus - New order");
                        msgForMerchant.AddTo(new EmailAddress("merchant@vodus.my"));
                        msgForMerchant.AddSubstitution("-orderTable-", item);
                        var responseMerchant = sendGridClientForMerchant.SendEmailAsync(msgForMerchant).Result;
                    }
