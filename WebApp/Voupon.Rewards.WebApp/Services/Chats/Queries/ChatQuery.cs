    ...
    public class ChatQueryHandler : IRequestHandler<ChatQuery, ApiResponseViewModel>
    {
        ...
        public async Task<ApiResponseViewModel> Handle(ChatQuery request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                var messages = await rewardsDBContext.ChatGroup.Include(x => x.ChatGroupUsers).Include(x => x.ChatMessages).Where(x => x.ChatGroupUsers.Any(y => y.UserId == request.UserId)).AsNoTracking().ToListAsync();
