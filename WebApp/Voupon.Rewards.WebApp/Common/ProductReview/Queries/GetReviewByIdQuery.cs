...
namespace Voupon.Rewards.WebApp.Common.ProductReview.Queries
{
    ...
    public class GetReviewByIdQuery : IRequest<ApiResponseViewModel>
    {
    ...
    public class GetReviewByIdQueryHandler : IRequestHandler<GetReviewByIdQuery, ApiResponseViewModel>
    {
        private readonly VodusV2Context vodusV2Context;
        RewardsDBContext rewardsDBContext;
        private readonly IOptions<AppSettings> appSettings;
        public GetReviewByIdQueryHandler(RewardsDBContext rewardsDBContext, VodusV2Context vodusV2Context, IOptions<AppSettings> appSettings)
        {
        ...
        public async Task<ApiResponseViewModel> Handle(GetReviewByIdQuery request, CancellationToken cancellationToken)
        {
            ApiResponseViewModel response = new ApiResponseViewModel();
            try
            {
                List<ProductReview> list = new List<ProductReview>();
                var item = await rewardsDBContext.ProductReview.Where(x => x.ProductId == request.ProductId).OrderByDescending(x=>x.CreatedAt).ToListAsync();
                if (item != null)
                {
