...
{
    public class ProductListByMerchantQuery : IRequest<ApiResponseViewModel>
    {
        public int MerchantId { get; set; }
        public string ExternalMerchantId { get; set; }
        public byte ExternalTypeId { get; set; }
    }
    public class ProductListByMerchantQueryHandler : IRequestHandler<ProductListByMerchantQuery, ApiResponseViewModel>
    {
        RewardsDBContext rewardsDBContext;
        private readonly IOptions<AppSettings> appSettings;
        public ProductListByMerchantQueryHandler(RewardsDBContext rewardsDBContext, IOptions<AppSettings> appSettings)
        {
            this.rewardsDBContext = rewardsDBContext;
            this.appSettings = appSettings;
        }

        public async Task<ApiResponseViewModel> Handle(ProductListByMerchantQuery request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                {
                    items = await rewardsDBContext.Products.Include(x => x.ProductOutlets).ThenInclude(x => x.Outlet).Include(x => x.Merchant).Include(x => x.ProductDiscounts).Include(x => x.ProductSubCategory).Include(x => x.ProductCategory).Include(x => x.StatusType).Include(x => x.DealType).Include(x => x.DealExpirations).Where(x => x.ExternalMerchantId == request.ExternalMerchantId && x.ExternalTypeId == request.ExternalTypeId && x.Merchant.IsPublished).ToListAsync();

                }
                else
                {
                    items = await rewardsDBContext.Products.Include(x => x.ProductOutlets).ThenInclude(x => x.Outlet).Include(x => x.Merchant).Include(x => x.ProductDiscounts).Include(x => x.ProductSubCategory).Include(x => x.ProductCategory).Include(x => x.StatusType).Include(x => x.DealType).Include(x => x.DealExpirations).Where(x => x.MerchantId == request.MerchantId && x.Merchant.IsPublished).ToListAsync();
                }
                ...
                foreach (var item in items)
                {
                    ...
                    newItem.ExternalTypeId = (item.ExternalTypeId.HasValue ? item.ExternalTypeId.Value : (byte)0);
                    ...
                    {
                        var higherDiscount = item.ProductDiscounts.Where(x => x.IsActivated == true).OrderByDescending(x => x.PointRequired).ToList().FirstOrDefault();
