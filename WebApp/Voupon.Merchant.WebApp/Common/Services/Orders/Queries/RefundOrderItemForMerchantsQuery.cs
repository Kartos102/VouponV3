...
{
    public class RefundOrderItemForMerchantsQuery : IRequest<ApiResponseViewModel>
    {
        public int Status { get; set; }

        public class RefundOrderItemForMerchantsQueryHandler : IRequestHandler<RefundOrderItemForMerchantsQuery, ApiResponseViewModel>
        {
            RewardsDBContext rewardsDBContext;
            public RefundOrderItemForMerchantsQueryHandler(RewardsDBContext rewardsDBContext)
            {
                this.rewardsDBContext = rewardsDBContext;
            }

            public async Task<ApiResponseViewModel> Handle(RefundOrderItemForMerchantsQuery request, CancellationToken cancellationToken)
            {
                ...
                try
                {
                    ...
                    */
                    ...
                    var deliveryPendingHandling = await rewardsDBContext.DeliveryRedemptionTokens.AsNoTracking().Include(x => x.OrderItem).ThenInclude(x => x.Order).Include(x => x.OrderItem).ThenInclude(x => x.Refunds).Where(x => (x.OrderItem.Status == 4 || x.OrderItem.Status == 5 || x.OrderItem.Status == 8 || x.OrderItem.Status == 9)).Select(x => new PendingHandlingItemViewModel
                    {
                        Id = x.Id,
                        ProductId = x.ProductId,
                        CreatedAt = x.CreatedAt,
                        ...
                        PromoCodeName = promoCodes.TryGetValue((Guid)x.OrderItem.Order.PromoCodeId, out promo) ? promo.Description : null,
                        MerchantName = x.OrderItem.MerchantDisplayName,
                        MerchantRefundAmount = x.OrderItem.Refunds.FirstOrDefault().MoneyRefunded,
                        MerchantRefundPoints = x.OrderItem.Refunds.FirstOrDefault().PointsRefunded,
                        MerchantRefundRemark = x.OrderItem.Refunds.FirstOrDefault().Remark
                    }).ToListAsync();

                    if (deliveryPendingHandling != null && deliveryPendingHandling.Any())
                    {
                        int tempProductId = 0;
                        var tempProductorderId = new Guid();
                        foreach (var deliveryItem in deliveryPendingHandling)
                        {
                            if(tempProductId == deliveryItem.ProductId && tempProductorderId == deliveryItem.OrderId)
                            {
                                continue;
                            }
                            ...
                            tempProductorderId = deliveryItem.OrderId;
                        }
        ...
        public class PendingHandlingItemViewModel
        {
            ...
            public decimal? Revenue { get; set; }
            public int MerchantRefundPoints { get; set; }
            public decimal? MerchantRefundAmount { get; set; }
            public string MerchantRefundRemark { get; set; }
            public string Email { get; set; }
