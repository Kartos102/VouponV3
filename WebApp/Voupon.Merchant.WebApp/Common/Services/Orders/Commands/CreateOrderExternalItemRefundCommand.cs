...
{
    public class CreateOrderExternalItemRefundCommand : IRequest<ApiResponseViewModel>
    {
        ...
        public class CreateOrderExternalItemRefundCommandHandler : IRequestHandler<CreateOrderExternalItemRefundCommand, ApiResponseViewModel>
        {
            RewardsDBContext rewardsDBContext;
            VodusV2Context vodusV2Context;
            IOptions<AppSettings> appSettings;
            public CreateOrderExternalItemRefundCommandHandler(RewardsDBContext rewardsDBContext, VodusV2Context vodusV2Context, IOptions<AppSettings> appSettings)
            {
            ...
            public async Task<ApiResponseViewModel> Handle(CreateOrderExternalItemRefundCommand request, CancellationToken cancellationToken)
            {
                ApiResponseViewModel apiResponseViewModel = new ApiResponseViewModel();

                var orderItem = await rewardsDBContext.OrderItemExternal.Include(x => x.OrderShopExternal).ThenInclude(x => x.Order).Where(x => x.Id == request.OrderItemId).FirstOrDefaultAsync();
                if (orderItem == null)
                {
                ...
                var user = await vodusV2Context.Users.Include(x => x.MasterMemberProfiles).Where(x => x.Email == orderItem.OrderShopExternal.Order.Email).FirstOrDefaultAsync();
                if (user == null)
                {
                    apiResponseViewModel.Successful = false;
                ...
                {
                    if (request.RefundAmount > (orderItem.TotalPrice + orderItem.OrderShopExternal.ShippingCost))
                    {
                        apiResponseViewModel.Successful = false;
                ...
                {
                    var id = Guid.NewGuid();
                    var refunds = new RefundsExternalOrderItems
                    {
                        Id = id,
                    ...
                    rewardsDBContext.RefundsExternalOrderItems.Add(refunds);
                    ...
                    orderItem.OrderItemExternalStatus = 5;
                    rewardsDBContext.OrderItemExternal.Update(orderItem);
                    await rewardsDBContext.SaveChangesAsync();

                    vodusV2Context.Database.CommitTransaction();
                    rewardsDBContext.Database.CommitTransaction();
                    var address = orderItem.OrderShopExternal.Order.ShippingAddressLine1 + ", ";
                    if (orderItem.OrderShopExternal.Order.ShippingAddressLine2 != null && orderItem.OrderShopExternal.Order.ShippingAddressLine2 != "")
                        address += orderItem.OrderShopExternal.Order.ShippingAddressLine2 + ", ";
                    address += orderItem.OrderShopExternal.Order.ShippingPostcode + " ";
                    address += orderItem.OrderShopExternal.Order.ShippingCity + " ";
                    address += orderItem.OrderShopExternal.Order.ShippingState + ", ";
                    address += orderItem.OrderShopExternal.Order.ShippingCountry;
                    ...
                    var orderTable = $"<table border='0' cellpadding='0' cellspacing='0' width='100%' class='x_478386741tableButton'>" +
                               ...
                               "<tr style = 'background-color:#eee;font-weight:bold;'>" +
                                ...
                                "<tr>" +
                                "<td width = '15%'>" +
                                    $"<img style = 'max-height:80px;max-width:80px;' id = '1624611321109100001_imgsrc_url_1' src='{orderItem.ProductCartPreviewSmallImageURL.Replace("http://", "https://")}'>" + "</td>" +
                    ...
                    var to = new EmailAddress(orderItem.OrderShopExternal.Order.Email, orderItem.OrderShopExternal.Order.BillingPersonFirstName + " " + orderItem.OrderShopExternal.Order.BillingPersonLastName);
                    ...
                    personalization.Substitutions.Add("-name-", orderItem.OrderShopExternal.Order.BillingPersonFirstName + " " + orderItem.OrderShopExternal.Order.BillingPersonLastName);
                    personalization.Substitutions.Add("-date-", orderItem.OrderShopExternal.Order.CreatedAt.ToString("dd MMM yyyy"));
                    ...
                    apiResponseViewModel.Message = "Succesfully refunded order item: #" + orderItem.ShortId + " You need to do manual refund by filling the form and send to RM";
                ...
                catch (Exception ex)
                {
                    ...
                    var errorLogs = new ErrorLogs
                    {
                        ...
                        CreatedAt = DateTime.Now,
                        Errors = ex.ToString(),
                        Email = orderItem.OrderShopExternal.Order.Email,
                        MasterProfileId = orderItem.OrderShopExternal.Order.MasterMemberProfileId
                    };
