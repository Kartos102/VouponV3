...
using Voupon.Merchant.WebApp.Common.Services.ProductCategories.Models;
using Voupon.Merchant.WebApp.ViewModels;

namespace Voupon.Merchant.WebApp.Common.Services.ProductCategories.Queries
{ 
    ...
    public class ProductCategoryListQueryHandler : IRequestHandler<ProductCategoryListQuery, ApiResponseViewModel>
    {
        ...
        public async Task<ApiResponseViewModel> Handle(ProductCategoryListQuery request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                var items = await rewardsDBContext.ProductCategories.AsNoTracking().Where(x=> x.IsActivated == true).ToListAsync();
