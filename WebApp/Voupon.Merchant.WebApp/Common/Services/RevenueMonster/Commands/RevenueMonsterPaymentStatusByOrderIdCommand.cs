...
using Voupon.Merchant.WebApp.Common.Services.RevenueMonster.Models;
using Voupon.Merchant.WebApp.Infrastructures.Helpers;
using Voupon.Merchant.WebApp.ViewModels;

namespace Voupon.Merchant.WebApp.Common.Services.RevenueMonster.Queries
{
    public class RevenueMonsterPaymentStatusByOrderIdCommand : IRequest<ApiResponseViewModel>
    {
        public string ShortId { get; set; }
        public bool GenerateOrderPayment { get; set; }

        public class RevenueMonsterPaymentStatusByOrderIdCommandHandler : IRequestHandler<RevenueMonsterPaymentStatusByOrderIdCommand, ApiResponseViewModel>
        {
            RewardsDBContext rewardsDBContext;
            private readonly IOptions<AppSettings> appSettings;
            public RevenueMonsterPaymentStatusByOrderIdCommandHandler(RewardsDBContext rewardsDBContext, IOptions<AppSettings> appSettings)
            {
            ...
            public async Task<ApiResponseViewModel> Handle(RevenueMonsterPaymentStatusByOrderIdCommand request, CancellationToken cancellationToken)
            {
                ...
                try
                {
                    ...
                    if (order == null)
                    {
                        ...
                        apiResponseViewModel.Message = "Invalid short id";
                    ...
                    if (status == "SUCCESS")
                    {
                        if (!request.GenerateOrderPayment)
                        {
                        ...
                        //  Regenerate order payment
                        httpClient = new HttpClient();
                        httpClient.DefaultRequestHeaders.Add("Authorization", $"Basic {Convert.ToBase64String(Encoding.UTF8.GetBytes($"{appSettings.Value.PaymentGateways.RevenueMonster.ClientId}:{appSettings.Value.PaymentGateways.RevenueMonster.ClientSecret}"))}");
                        var paymentData = new StringContent(jsonAsString, Encoding.UTF8, "application/json");
                        var createPayment = await httpClient.PostAsync($"{appSettings.Value.App.VouponUrl}/api/v1/thirdparty/revenuemonster/regenFromAdmin", paymentData);
                        //var createPayment = await httpClient.PostAsync($"https://localhost:5001/api/v1/thirdparty/revenuemonster/regenFromAdmin", paymentData);
                        string createPaymentResult = createPayment.Content.ReadAsStringAsync().Result;
