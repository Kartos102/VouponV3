...
{  
    public class TestMerchantProductListQuery : IRequest<ApiResponseViewModel>
    {
    }
    public class TestMerchantProductListQueryHandler : IRequestHandler<TestMerchantProductListQuery, ApiResponseViewModel>
    {
        RewardsDBContext rewardsDBContext;
        public TestMerchantProductListQueryHandler(RewardsDBContext rewardsDBContext)
        {
            this.rewardsDBContext = rewardsDBContext;
        }

        public async Task<ApiResponseViewModel> Handle(TestMerchantProductListQuery request, CancellationToken cancellationToken)
        {
            ApiResponseViewModel response = new ApiResponseViewModel();
            try
            {
                var items = await rewardsDBContext.Products.Include(x => x.Merchant).Include(x => x.ProductSubCategory).Include(x => x.ProductCategory).Include(x => x.StatusType).Include(x => x.DealType).Where(x=>x.Merchant.IsTestAccount == true).ToListAsync();
