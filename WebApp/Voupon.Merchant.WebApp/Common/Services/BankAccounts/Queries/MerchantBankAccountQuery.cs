...
{
    public class MerchantBankAccountQuery : IRequest<ApiResponseViewModel>
    {
        public int MerchantId { get; set; }
    }

    public class MerchantBankAccountQueryHandler : IRequestHandler<MerchantBankAccountQuery, ApiResponseViewModel>
    {
        RewardsDBContext rewardsDBContext;
        public MerchantBankAccountQueryHandler(RewardsDBContext rewardsDBContext)
        {
            this.rewardsDBContext = rewardsDBContext;
        }

        public async Task<ApiResponseViewModel> Handle(MerchantBankAccountQuery request, CancellationToken cancellationToken)
        {
            ApiResponseViewModel response = new ApiResponseViewModel();
            try
            {
                BankAccountModel bankAccountModel = new BankAccountModel();
                var bankAccount = await rewardsDBContext.BankAccounts.Include(x => x.Bank).Include(x => x.StatusType).FirstOrDefaultAsync(x => x.MerchantId == request.MerchantId);               
                if (bankAccount != null)
                {
